...
export default {
    components: { Layout, DatePicker, Multiselect, countTo },
    ...
    methods: {
        ...
        formBuscar() {
            ...
            console.log(this.formbuscar.fecha)

            if (Array.isArray(this.formbuscar.fecha) && (this.formbuscar.fecha[0] || this.formbuscar.fecha[1])) {


                this.formbuscar.id_sucursal = this.sucursal.id_sucursal;
                this.formbuscar.fecha_inicio = moment(
                    this.formbuscar.fecha[0]
                ).format("YYYY-MM-DD");
                this.formbuscar.fecha_fin = moment(
                    this.formbuscar.fecha[1]
                ).format("YYYY-MM-DD");

                this.axios
                    .post(`/api/buscarinformemensual`, this.formbuscar)
                    .then((res) => {
                        console.log(res);
                        if (res.data) {
                            let total = 0;
                ...
                this.sweealerta('error', 'Debe seleccionar un rango de fecha')
            }
        },
        ver(data) {
            console.log(data);
